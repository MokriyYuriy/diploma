...
class BiLSTMDiscriminator(nn.Module):
    def __init__(self, alphabet, embedding_size, hidden_size, alpha):
        super(BiLSTMDiscriminator, self).__init__()
        self.hidden_size = hidden_size
        self.alphabet = alphabet
        self.alpha = alpha
        self.embedding = nn.Embedding(embedding_dim=embedding_size, num_embeddings=len(alphabet))
    ...
    def forward(self, input_sequence):
        ...
        mask = self.alphabet.get_mask(input_sequence)
        ...
        forward_hs = forward_out[range(batch_size), self.alphabet.get_length(input_sequence) - 1]
        backward_hs = backward_out[range(batch_size), self.alphabet.get_length(input_sequence) - 1]
